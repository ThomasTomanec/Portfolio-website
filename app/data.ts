...
export async function getSocialAccounts(username: string): Promise<any> {
...
export const getPinnedRepos = cache(async (username: string): Promise<string[]> => {
...
export const getUserOrganizations = async (username: string): Promise<any> => {
...
export const getLanguage = async (username: string): Promise<any> => {
    console.log('Fetching organizations for', username);
...
export const getNextjsLatestRelease = cache(async (): Promise<any> => {
    const res = await fetch('https://api.github.com/graphql', {
        method: 'POST',
        headers: { Authorization: `Bearer ${process.env.GH_TOKEN}` },
        body: JSON.stringify({
...
export const getRepositoryPackageJson = cache(async (username: string, reponame: string): Promise<any> => {
    const res = await fetch('https://api.github.com/graphql', {
        method: 'POST',
        headers: { Authorization: `Bearer ${process.env.GH_TOKEN}` },
        body: JSON.stringify({
...
export const getRecentUserActivity = cache(async (username: string): Promise<any> => {
...
export const getTrafficPageViews = async (username: string, reponame: string): Promise<any> => {
    ...
    const todayUniques = response.views?.find((day: any) => day.timestamp.startsWith(today))?.uniques || 0;

    return { sumUniques, todayUniques };
};

export const getDependabotAlerts = cache(async (username: string, reponame: string): Promise<any> => {
 ...
 */
export async function checkAppJsxExistence(repoOwner: string, repoName: string): Promise<any> {
